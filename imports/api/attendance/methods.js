...
Meteor.methods({
  ...
  async "staffsAttendance.getRekapByUser"(userId, monthData) {
    ...
    if (userMonthly) {
      ...
      const presentaseKehadiran =
        (userMonthly.totalPresensi / monthly.activeDayWorking) * 100;
  ...
  "staffsAttendance.rekap"(totalDayOf, startDate2, endDate2) {
    ...
    const totalDay = moment().weekdayCalc(startDate, endDate, [1, 2, 3, 4, 5]);
              ...
              if (dataStaffsAttendance) {
                ...
                permitAbsence.forEach((doc) => {
                  doc.datesPermits.forEach((datePermits) => {
                    ...
                    if (
                      permitDate >= startOfMonth &&
                      permitDate <= endOfMonth
                    ) {
                      totalPermit += 1;
                    }
                  });
  ...
  "update.scheduleAttendance"(id, name, dataSend) {
    ...
    return ScheduleAttendance.update({ _id: id }, { $set: dataSave });
  },
