...
Meteor.methods({
  async "users.getAll"() {
    ...
    const listUsers = await Meteor.users
      .find({ roles: { $in: ["adminPpdbYayasan", "adminPpdbSchool"] } })
      .fetch();

    listUsers.map((item) => {
      if (item.schoolId) {
        const school = Schools.findOne({ _id: item.schoolId });
        if (school) {
          item.schoolName = school.name + " | " + school.unitName;
        }
      }
    });
    return listUsers;
    ...
    return Meteor.users.update(
      { _id: id },
      {
        $set: {
          ...
          "emails.0.address": dataSave.username,
          fullname: dataSave.fullname,
          roles: [dataSave.roles],
        },
      }
    );
  },
  ...
  "users.getDataNotEmployee"() {
    ...
    for (let index = 0; index < dataAppProfile.length; index++) {
      ...
      if (dataAppUser) {
        const dataEmployee = Employee.findOne({
          email_address: dataAppUser.email,
        });
        if (!dataEmployee) {
  ...
  "users.createDataEmployee"(id) {
    ...
    const objectIdAppProfile = new Meteor.Collection.ObjectID(
      dataAppuser.profileId
    );
  ...
  async "users.updateProfileIdAppUser"(data) {
    ...
    for (const iterator of data) {
      ...
      const getAppProfiles = await AppProfiles.findOne({
        _id: objectIdProfile,
      });
      ...
      if (
        getEmployee.isUpdated == undefined ||
        getEmployee.isUpdated == null ||
