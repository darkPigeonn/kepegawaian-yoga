...
Template.createPayroll.events({
      ...
      function (error, result) {
        if (result) {
          const persentaseKehadiran = (
            (result.details.totalPresensi / result.activeDayWorking) *
            100
          ).toFixed(2);
  ...
  "click #btn-save-lembur"(e, t) {
    ...
    const name =
      `Tarif Lembur @Rp` + formatRupiah(t.rateOvertime.get().toString());
